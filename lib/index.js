...
function piRatio(Xi, Xj, options) {
  ...
  var centeri = Math.floor(Math.random() * options.number);
  var centerj = Math.floor(Math.random() * options.number);
  var thetai = theta(Xi, options.r, centeri);
  var thetaj = theta(Xj, options.r, centerj);
  var pjpi = Math.exp((thetai - thetaj) / options.t);
  return pjpi;
}
...
function acceptReject(Xi, Xj, options) {
  ...
  var A = (piRatio(Xi, Xj, options) * proposal(Xi, Xj)) / proposal(Xj, Xi);
  var R = Math.random();
  if (Math.min(A, 1) > R) {
    return Xj;
  }
  ...
  */
  ...
  while (step < options.iterate) {
    ...
    if (Math.random() > 0.5 && Xi.edges().length < edges) {
      ...
      step++;
      addRandomEdge(Xj);
      Xi = copyGraph(acceptReject(Xi, Xj, options), coor);
