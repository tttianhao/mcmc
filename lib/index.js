...
function proposal(Xj, Xi) {
  ...
  var B = 0;
  for (var i = 0; i < Xj.edges().length; i++) {
    if (isBridge(Xj, Xj.edges()[i][0], Xj.edges()[i][1])) {
      B++;
    }
  }
...
var main = function(options) {
  ...
  console.log(matrix);
  console.log(theta(G, r, source));
  console.log(piRatio(G, G, [r, T], source));
  console.log(addOrDelete(G));
  console.log(proposal(G, G));
  var G1 = copyGraph(G, corArray);
  console.log(G1.edges(true));
  console.log(G1.nodes(true));
  addRandomEdge(G1);
  console.log(G.edges());
  console.log(G1.edges());
  // Var Xi = Object.assign(G1, G);
  // var Xj = Object.assign(G1, G);
};
